...
```bash
docker run [...] -v <PATH>/config.json:/config.json shounak9642/watchtower
```
...
```bash
docker run [...] -v <PATH_TO_HOME_DIR>/.docker/config.json:/config.json shounak9642/watchtower
```
...
services:
  watchtower:
    image: shounak9642/watchtower:latest
    volumes:
